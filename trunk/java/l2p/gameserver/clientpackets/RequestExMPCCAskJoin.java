...
public class RequestExMPCCAskJoin extends L2GameClientPacket {
	...
	public void runImpl() {
		...
		// Сам себя нельзя
		if((activeChar == target) || (target.getParty().getPartyLeader() == activeChar)) {
			return;
		}
		...
		} else if(L2CommandChannel.checkAuthority(activeChar) || activeChar.getInventory().getItemByItemId(L2CommandChannel.STRATEGY_GUIDE_ID) != null) { // СС еще не существует. Отсылаем запрос на инвайт и в случае согласия создаем канал
				sendInvite(activeChar, target);
		}
	}
