...
public final class L2GameClient extends MMOClient<MMOConnection<L2GameClient>>
{
	...
	protected void onDisconnection()
	{
		...
		try
		{
			...
			{
				Object[] script_args = new Object[] {player.getObjectId()};
				for(Scripts.ScriptClassAndMethod handler : Scripts.onDisconnect)
				{
	...
	public boolean decrypt(ByteBuffer buf, int size)
	{
		...
		if(!ret && protect_used)
		{
			Log.add("PacketHack from " + toString() + " | HWID: " + HWID, "protect");
			if(_activeChar != null)
			{
				_activeChar.logout(false, false, true, true);
			}
