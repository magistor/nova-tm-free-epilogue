...
public final class L2Player extends L2Playable
{
	...
	public void prepareToLogout()
	{
		...
		Object[] script_args = new Object[] {_objectId};
	...
	public static L2Player restore(final int objectId)
	{
		...
		try
		{
			...
			if(pl_rset.next() && ps_rset.next())
			{
				...
				if((player.getVar("jailed") != null) && (System.currentTimeMillis() < Long.parseLong(player.getVar("jailed"))))
				{
	...
	*/
	...
	{
		return getVitalityLevel() * Config.ALT_VITALITY_POWER_MULT / 2f;
	}
	...
	private void checkReferralBonus(int id)
	{
		...
		try
		{
			...
			if(Config.PROTECT_ENABLE)
			{
				...
				{
					return;
				} // other char not exist or hwid invalid
			...
			L2ItemInstance reward1 = ItemTable.getInstance().createItem(id == 1 ? Config.SERVICES_REFERRAL_ITEM_1 : Config.SERVICES_REFERRAL_ITEM_2);
			...
			L2ItemInstance reward2 = ItemTable.getInstance().createItem(id == 1 ? Config.SERVICES_REFERRAL_ITEM_1 : Config.SERVICES_REFERRAL_ITEM_2);
	...
	public FastList<TeleportPoint> teleportPoints;
	public TeleportPoint getTeleportPointByName(String name)
	{
