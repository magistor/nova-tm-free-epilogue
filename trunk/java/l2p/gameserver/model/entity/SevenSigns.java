...
import l2p.gameserver.model.AutoSpawnHandler.AutoSpawnInstance;
...
public class SevenSigns
{
	...
	public SevenSigns()
	{
		...
		{
			ThreadPoolManager.getInstance().scheduleGeneral(new SevenSignsAnnounce(), Config.SS_ANNOUNCE_PERIOD * 1000L * 60);
		}
	}
	...
	public class SevenSignsPeriodChange implements Runnable
	{
		public void run()
		{
			...
			try
			{
				...
				setCalendarForNextPeriodChange();
				_log.config("SevenSignsPeriodChange: time to next change=" + Util.formatTime(getMilliToPeriodChange() / 1000));
	...
	public void changePeriod(int period, int seconds)
	{
		...
		_periodChange = ThreadPoolManager.getInstance().scheduleGeneral(new SevenSignsPeriodChange(), seconds * 1000L);
	}
