...
import l2p.gameserver.model.L2Skill.Element;
...
import l2p.gameserver.skills.conditions.ConditionPlayerState;
import l2p.gameserver.skills.conditions.ConditionPlayerState.CheckPlayerState;
...
public class Formulas
{
	...
	static
	{
		...
		File file = new File(Config.AttributeBonusFile);
		Document doc = null;
		try
		{
	...
	public static double calcHpRegen(L2Character cha)
	{
		...
		else if(cha.isMinion() && ((L2MinionInstance) cha).isRaidFighter())
		{
			init *= cha.getLevel() / 6;
		}
		if(cha.isPlayable())
	...
	public static boolean calcBlow(L2Character activeChar, L2Character target, L2Skill skill)
	{
		...
		if(Config.SKILLS_SHOW_CHANCE && activeChar.isPlayer() && !((L2Player) activeChar).getVarB("SkillsHideChance")) // Выводим сообщение с шансом
		{
			activeChar.sendMessage(new CustomMessage("l2p.gameserver.skills.Formulas.Chance", activeChar).addString(skill.getName()).addNumber((long) chance));
		}
	...
	public static boolean calcSkillSuccess(Env env, Stats resistType, Stats attibuteType, int spiritshot)
	{
		...
		if(skill.isMagic()) // Этот блок только для магических скиллов
		{
			...
			env.value *= Config.SKILLS_CHANCE_MOD * Math.pow(matk, Config.SKILLS_CHANCE_POW) / mdef;
		}
		...
		if(target.isPlayer())
		{
			...
			if((Config.SKILLS_SHOW_CHANCE && character.isMonster() || player.isGM()) && player.getVarB("SkillsMobChance"))
			{
				target.sendMessage(character.getName() + ": " + new CustomMessage("l2p.gameserver.skills.Formulas.Chance", target).addString(skill.getName()).addNumber(Math.round(env.value)).toString());
			}
		}
		if(Config.SKILLS_SHOW_CHANCE || isGM)
		{
			...
			{
				player.sendMessage(new CustomMessage("l2p.gameserver.skills.Formulas.Chance", player).addString(skill.getName()).addNumber(Math.round(env.value)));
			}
		}
		return Rnd.chance(env.value);
	}
	...
	public static double calcDamageResists(L2Skill skill, L2Character attacker, L2Character defender, double value)
	{
		...
		{
			return value;
		} // TODO: по хорошему надо учитывать защиту, но поскольку эти скиллы немагические то надо делать отдельный механизм
		if(attacker.isBoss())
		{
