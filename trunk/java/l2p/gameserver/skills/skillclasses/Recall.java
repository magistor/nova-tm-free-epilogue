...
public class Recall extends L2Skill
{
	...
	public boolean checkCondition(L2Character activeChar, L2Character target, boolean forceUse, boolean dontMove, boolean first)
	{
		...
		if(activeChar.isPlayer())
		{
			...
			{
				p.sendMessage(new CustomMessage("l2p.gameserver.skills.skillclasses.Recall.Here", p));
				return false;
			}
		}
		if(activeChar.isInZone(ZoneType.no_escape) || _townId > 0 && activeChar.getReflection().getCoreLoc() != null)
		{
			activeChar.sendMessage(new CustomMessage("l2p.gameserver.skills.skillclasses.Recall.Here", activeChar));
			return false;
		}
	...
	public void useSkill(final L2Character activeChar, GArray<L2Character> targets)
	{
		for(final L2Character target : targets)
		{
			if(target != null)
			{
				...
				if(pcTarget.isFlying() || pcTarget.isInFlyingTransform())
				{
					continue;
				}
				...
				{
					activeChar.sendMessage(new CustomMessage("l2p.gameserver.skills.skillclasses.Recall.Festival", activeChar));
					continue;
				}
