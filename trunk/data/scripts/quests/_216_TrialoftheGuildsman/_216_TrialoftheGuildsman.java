...
public class _216_TrialoftheGuildsman extends Quest implements ScriptFile
{
	...
	private static final int[][] DROPLIST_COND = {{3, 4, 20223, VALKONS_RECOMMEND, MANDRAGORA_BERRY, 1, 20, 1},
	...
	public _216_TrialoftheGuildsman()
	{
		...
		addQuestItem(new int[] {ALLTRANS_INSTRUCTIONS, VALKONS_RECOMMEND, ALLTRANS_RECOMMEND1, NORMANS_INSTRUCTIONS,
			NORMANS_LIST, NORMANS_RECEIPT, ALLTRANS_RECOMMEND2, PINTERS_INSTRUCTIONS, DUNINGS_INSTRUCTIONS, JOURNEYMAN_GEM,
			JOURNEYMAN_DECO_BEADS, JOURNEYMAN_RING, AMBER_BEAD, AMBER_LUMP, MANDRAGORA_BERRY, DUNINGS_KEY,
			GRAY_BONE_POWDER, RED_PIGMENT, GRANITE_WHETSTONE, BRAIDED_YARN});
	}
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("30103_3") || event.equalsIgnoreCase("30103_4"))
		{
			...
			if(!st.getPlayer().getVarB("prof2.1"))
			{
				...
				st.giveItems(57, 93803, Config.RATE_QUESTS_OCCUPATION_CHANGE);
		...
		if(cond == 5 && (npcId == 20079 || npcId == 20080 || npcId == 20081))
		{
			if(Rnd.chance(33) && st.getQuestItemsCount(ALLTRANS_INSTRUCTIONS) > 0 && st.getQuestItemsCount(PINTERS_INSTRUCTIONS) > 0)
			{
				...
				if(count < 70 && st.getPlayer().getClassId().getId() == 0x38)
				{
					...
					if(((L2MonsterInstance) npc).isSpoiled())
					{
						st.giveItems(AMBER_LUMP, 5);
					}
