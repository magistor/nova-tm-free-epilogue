...
public class _230_TestOfSummoner extends Quest implements ScriptFile
{
	...
	static String[] STATS = {"cond", "step", "Lara_Part", "Arcanas", "Belthus", "Brynthea", "Celestiel", "Camoniell",
		"Basilla", "Almors"};
			...
			{ // Start&&End Npc Galatea related stuff
				...
				{ // step 2 means already talkd with lara
					...
					{ // finished all battles... the player is able to earn the marks
						...
						{
							st.addExpAndSp(832247, 57110, true);
							st.giveItems(ADENA_ID, 150480, Config.RATE_QUESTS_OCCUPATION_CHANGE);
	...
	static
	{
		DROPLIST_SUMMON.put(27102, new Integer[] {CRYSTAL_OF_PROGRESS1_ID, CRYSTAL_OF_INPROGRESS1_ID, CRYSTAL_OF_FOUL1_ID,
			CRYSTAL_OF_DEFEAT1_ID, CRYSTAL_OF_VICTORY1_ID}); // Pako the Cat
		DROPLIST_SUMMON.put(27103, new Integer[] {CRYSTAL_OF_PROGRESS2_ID, CRYSTAL_OF_INPROGRESS2_ID, CRYSTAL_OF_FOUL2_ID,
			CRYSTAL_OF_DEFEAT2_ID, CRYSTAL_OF_VICTORY2_ID}); // Mimi the Cat
		DROPLIST_SUMMON.put(27104, new Integer[] {CRYSTAL_OF_PROGRESS3_ID, CRYSTAL_OF_INPROGRESS3_ID, CRYSTAL_OF_FOUL3_ID,
			CRYSTAL_OF_DEFEAT3_ID, CRYSTAL_OF_VICTORY3_ID}); // Shadow Turen
		DROPLIST_SUMMON.put(27105, new Integer[] {CRYSTAL_OF_PROGRESS4_ID, CRYSTAL_OF_INPROGRESS4_ID, CRYSTAL_OF_FOUL4_ID,
			CRYSTAL_OF_DEFEAT4_ID, CRYSTAL_OF_VICTORY4_ID}); // Unicorn Racer
		DROPLIST_SUMMON.put(27106, new Integer[] {CRYSTAL_OF_PROGRESS5_ID, CRYSTAL_OF_INPROGRESS5_ID, CRYSTAL_OF_FOUL5_ID,
			CRYSTAL_OF_DEFEAT5_ID, CRYSTAL_OF_VICTORY5_ID}); // Unicorn Phantasm
		DROPLIST_SUMMON.put(27107, new Integer[] {CRYSTAL_OF_PROGRESS6_ID, CRYSTAL_OF_INPROGRESS6_ID, CRYSTAL_OF_FOUL6_ID,
			CRYSTAL_OF_DEFEAT6_ID, CRYSTAL_OF_VICTORY6_ID}); // Silhoutte Tilfo
	}
	...
	@Override
	public String onDeath(L2Character killer, L2Playable victim, QuestState st)
	{
	...
	{ // on the first attack, the stat is in battle... anytime gives crystal and set stat
		...
		if(npcId >= 27102 && npcId <= 27107)
		{
			...
			GArray<AggroInfo> attackerList = npc.getAggroList();
			L2Summon summon = st.getPlayer().getPet();
			...
			{
outer:
				for(AggroInfo a : attackerList)
				{
					if(a.attacker == summon)
					{
						continue;
					}
					if(a.attacker == st.getPlayer())
					{
						for(AggroInfo ai : attackerList)
						{
							if(ai.attacker == summon)
							{
								if(ai.damage == a.damage)
								{
									continue outer;
								}
								else
