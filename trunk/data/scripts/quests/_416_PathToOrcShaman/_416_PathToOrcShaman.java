...
public class _416_PathToOrcShaman extends Quest implements ScriptFile
{
	...
	private static final int[][] DROPLIST_COND = {{1, 0, KashaBear, FireCharm, KashaBearPelt, 1, 70, 1},
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		{
			for(L2NpcInstance n : L2ObjectsStorage.getAllByNpcId(QuestMonsterDurkaSpirit, false))
			{
				n.deleteMe();
			}
		}
		...
		else if(npcId == SeerMoira)
		{
			...
			else if(cond == 21)
			{
				...
				if(st.getPlayer().getClassId().getLevel() == 1)
				{
					...
					{
						st.getPlayer().setVar("prof1", "1");
						st.addExpAndSp(295862, 18194, true);
						if(HWID.getBonus(st.getPlayer(), "newbie5", L2NewbieGuideInstance.BonusComparator) == 0)
						{
							st.giveItems(ADENA_ID, 81900, Config.RATE_QUESTS_OCCUPATION_CHANGE);
							HWID.setBonus(st.getPlayer().getHWID(), "newbie5", 1);
	...
	public String onKill(L2NpcInstance npc, QuestState st)
	{
		...
		else if(cond == 9 && (npcId == VenomousSpider || npcId == ArachnidTracker))
		{
			...
			{
				if(L2ObjectsStorage.getByNpcId(QuestMonsterDurkaSpirit) == null)
				{
		...
		else if(npcId == QuestBlackLeopard)
		{
			...
			{
				Functions.npcSayCustomMessage(L2ObjectsStorage.getByNpcId(LeopardCarcass), new CustomMessage("quests._416_PathToOrcShaman.LeopardCarcass", st.getPlayer()).toString(), st.getPlayer());
				st.setCond(15);
			}
