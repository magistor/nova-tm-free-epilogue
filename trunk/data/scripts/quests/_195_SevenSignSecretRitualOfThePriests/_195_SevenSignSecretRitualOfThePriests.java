...
public class _195_SevenSignSecretRitualOfThePriests extends Quest implements ScriptFile
{
	...
	private static final int TestItem = 750;
	...
	public String onTalk(L2NpcInstance npc, QuestState st)
	{
		...
		if(npcId == IdentityConfirmDevice)
		{
			if(player.getTransformation() == GuardsOfTheDawn && st.getQuestItemsCount(GuardsoftheDawnIdentityCard) != 0)
			{
				if(st.getQuestItemsCount(TestItem) > 0)
				{
					...
					st.takeItems(TestItem, st.getQuestItemsCount(TestItem));
