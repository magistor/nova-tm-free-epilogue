...
public class _402_PathToKnight extends Quest implements ScriptFile
{
	...
	public final int[][] DROPLIST = {{BUGBEAR_RAIDER, GLUDIO_GUARDS_MARK1, BUGBEAR_NECKLACE, 10, 100},
	...
	public _402_PathToKnight()
	{
		...
		addQuestItem(new int[] {BUGBEAR_NECKLACE, EINHASAD_CRUCIFIX, POISON_SPIDER_LEG1, LIZARDMAN_TOTEM,
			GIANT_SPIDER_HUSK, HORRIBLE_SKULL});
	}
	...
	public String onKill(L2NpcInstance npc, QuestState st)
	{
		...
		for(int[] element : DROPLIST)
		{
			if(st.getInt("cond") > 0 && npcId == element[0] && st.getQuestItemsCount(element[1]) > 0 && st.getQuestItemsCount(element[2]) < element[3] && Rnd.chance(element[4]))
			{
