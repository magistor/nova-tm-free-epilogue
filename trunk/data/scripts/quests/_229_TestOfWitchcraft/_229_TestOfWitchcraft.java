...
public class _229_TestOfWitchcraft extends Quest implements ScriptFile
{
	...
	private static final int[][] DROPLIST_COND = {{2, 0, DireWyrm, IkersList, DireWyrmFang, 20, 100, 1},
	...
	public _229_TestOfWitchcraft()
	{
		...
		addQuestItem(new int[] {OrimsDiagram, OrimsInstructions, Orims1stLetter, Orims2stLetter, Brimstone_1st,
			AlexandriasBook, IkersList, Aklantoth_1stGem, SoultrapCrystal, IkersAmulet, Aklantoth_2stGem, LarasMemo,
			NestlesMemo, LeopoldsJournal, Aklantoth_4stGem, Aklantoth_5stGem, Aklantoth_6stGem, SirVaspersLetter,
			SwordOfBinding, VadinsCrucifix, VadinsSanctions, Brimstone_2nd, PurgatoryKey, ZeruelBindCrystal, DireWyrmFang,
			EnchantedGolemHeartstone, LetoLizardmanCharm, Aklantoth_3stGem, TamlinOrcAmulet});
	}
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("30630-14.htm"))
		{
			...
			else
			{
				...
				L2NpcInstance Zeruel = st.addSpawn(DrevanulPrinceZeruel);
				if(Zeruel != null)
				{
					st.getPlayer().addDamageHate(Zeruel, 0, 1);
					Zeruel.getAI().setIntention(CtrlIntention.AI_INTENTION_ATTACK, st.getPlayer());
				}
			}
		}
		...
		else if(event.equalsIgnoreCase("30630-22.htm"))
		{
			...
			{
				st.addExpAndSp(1029122, 70620, true);
				st.giveItems(ADENA_ID, 186077, Config.RATE_QUESTS_OCCUPATION_CHANGE);
